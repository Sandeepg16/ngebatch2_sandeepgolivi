...
const queryUser = (personName: string) => new Promise((resolve, reject) => {
  const result = userData.filter((user) => user.name === personName)[0] || null
  resolve(result);
})
...
const queryFood = (foodId: number) => new Promise((resolve, reject) => {
  const result = foodData.filter((food) => food.id === foodId)[0] || null
  resolve(result);
})
...
const findFavouriteFood = (name: string): Promise<string> => {
  return new Promise((resolve, reject) => {
    queryUser(name)
      ...
      .then((foodItem: any) => {
        resolve(`${name} likes ${foodItem.name}`);
      })
      .catch(error => {
        reject(error);
      });
...
queryUser('John').then((person:any) => queryFood(person.food)).then((foodItem:any) => console.log(`xxx likes ${foodItem.name}`)) // xxx likes Pizza
